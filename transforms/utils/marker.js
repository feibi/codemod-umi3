...
const iconMarkerPath = path.join(require('os').tmpdir(), './antd-v4-icon.marker.txt');

const compatibleMarkerPath = path.join(require('os').tmpdir(), './antd4-v4-compatible.marker.txt');
...
async function start() {
  return await Promise.all(
    Object.values(dependencyMarkerPathMap).map((markPath) => fsOpenAsync(markPath, 'w')),
  );
}
