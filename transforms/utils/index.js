...
function insertImportBefore(j, root, {importStatement, beforeModule}) {
  ...
  if (beforeModule && firstBeforeModuleImport) {
...
function removeEmptyModuleImport(j, root, moduleName) {
  root
    .find(j.ImportDeclaration)
    .filter((path) => {
      return path.node.specifiers.length === 0 && path.node.source.value === moduleName;
    })
    .replaceWith();
}
...
function addSubmoduleImport(j, root, {moduleName, importedName, localName, before}) {
